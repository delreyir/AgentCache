...
Publish: Agent A encodes a JSON blob, registers the commitment on Aptos, and uploads it to Shelby via shelbyClient.rpc.putBlob().

Buy: Agent B sends APT to the AgentCache Smart Contract.

Unlock: Agent B requests the blob from Shelby RPC. Shelby verifies the on-chain payment on Aptos and returns the decrypted payload instantly.